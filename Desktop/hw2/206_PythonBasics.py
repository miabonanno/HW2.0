...
# Section Day/Time: # Wed/6-7
# People you worked with: #Emily Behnana, Bianca Emde, Sydney Hunt
...
def string_manip(s):
    space = s.strip()
    capital = space.upper()
    replacement = capital.replace(' ', '#')
    deleteSI = replacement.replace('UMSI','')
    if len(deleteSI) == 1:
        return deleteSI
    else:
        return deleteSI[::-1]
...
def name_counts(names):
    ...
    for x in names:
        if x not in d:
            ...
            d[x] +=1
    beginning= d.items()
    list1= sorted(beginning, key= lambda x: x[0])
    final= sorted(list1, key=lambda x: x[1], reverse= True)
    return final
